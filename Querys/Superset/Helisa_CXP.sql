SELECT 
    ...
    SF.fecha AS "Fecha",
    (SF.periodo || SUBSTRING(SF.fecha FROM 1 FOR 2)) AS "Fecha AMD",
    TRIM(SF.Nit) AS "Nit Proveedor",
    PRO.Nombre AS "Nombre Proveedor",
    SF.cuenta AS "Cuenta",
    CTA.NOMBRE AS "Nombre Cuenta",
    SF.TipodeDocumento AS "Tipo de Documento",
    DOCUXXXX.NOMBRE AS "Nombre Tipo de Documento",
    SF.Documento AS "Documento",
    SF.concepto AS "Concepto",
    SF.centroc AS "Centro de Costo",
    CCO.NOMBRE AS "Nombre Centro de Costo",
    SF.factura AS "Valor Factura",
    SF.saldodoc AS "Saldo Factura",
    CAST(HEDATETOSTR(SF.fecha_vence,'yyyy/mm/dd') AS DATE) AS "Fecha Vencimiento",
    SF.mora AS "Dias Mora",
    SF.por_vencer AS "Saldo Por Vencer",
    SF.cuentasxp1a30 AS "C X Pagar de 1 a 30 dias",
    SF.cuentasxp31a60 AS "C X Pagar de 31 a 60 dias",
    SF.cuentasxp61a90 AS "C X Pagar de 61 a 90 dias",
    SF.cuentasxp91a120 AS "C X Pagar de 91 a 120 dias",
    SF.cuentasxp121a150 AS "C X Pagar de 121 a 150 dias",
    SF.cuentasxpmayor90 AS "C X Pagar Mayor a 90 dias",
    SF.cuentasxpmayor120 AS "C X Pagar Mayor a 120 dias",
    SF.cuentasxpmayor150 AS "C X Pagar Mayor a 150 dias",
    SF.cuentasxpmayor180 AS "C X Pagar Mayor a 180 dias"
